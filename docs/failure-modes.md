...
[`backend.py:103`](../src/ragsage/parsing/backend.py), popped at `backend.py:126`), so call
...
([`pdf.py:304`](../src/ragsage/parsing/pdf.py)), which finds columns by locating the vertical
...
- **Any full-width element whose words cross the gutter** — a centred title, a wide figure
  ...
  ([`pdf.py:259`](../src/ragsage/parsing/pdf.py)) then merges left- and right-column lines
...
- **A column that stops early** — the last page of an article, a short sidebar — fails
  `_both_sides_span_height` ([`pdf.py:342`](../src/ragsage/parsing/pdf.py)), which requires
...
- **Supply a flow format instead.** DOCX, PPTX and HTML have no column geometry to misread;
  those paths walk native structure ([`docx.py`](../src/ragsage/parsing/docx.py),
  [`html.py`](../src/ragsage/parsing/html.py)) and are unaffected by this entry entirely.
- **Re-export the PDF single-column** if it is yours to re-export.
- **Bring your own parser.** `DocumentParser` is a `Protocol`
  ([`ports.py:48`](../src/ragsage/ports.py)); see
  [`examples/custom_parser.py`](../examples/custom_parser.py). You can parse the hard
...
trusted outright (`_RULED_SETTINGS` [`pdf.py:66`](../src/ragsage/parsing/pdf.py),
`_ruled_tables` `pdf.py:415`). Everything else goes through `_borderless_tables`
([`pdf.py:430`](../src/ragsage/parsing/pdf.py)), which is inference over whitespace:
...
- **The cell boundary is one fixed distance, applied per line.** A line splits into cells
  ...
  `_split_cells` [`pdf.py:470`](../src/ragsage/parsing/pdf.py)) — nothing aligns the splits
...
- **A widely set table can be torn up before the detector even runs.** Its inter-column gaps
  ...
  columns and `_serialize` ([`pdf.py:179`](../src/ragsage/parsing/pdf.py)) emits each band
...
- **A blank cell ends the table.** A table is only a table where consecutive lines split into
  ...
  **one** row, and the last two rows leave the table entirely. Note too that the first line of
  ...
  [`pdf.py:484`](../src/ragsage/parsing/pdf.py)) — so where a run starts below the real
...
- **Supply DOCX, PPTX or HTML.** Those paths read real table grids
  (`_render_table` [`docx.py:185`](../src/ragsage/parsing/docx.py),
  [`html.py:197`](../src/ragsage/parsing/html.py)) rather than guessing from whitespace.
...
([`ingestion.py:57`](../src/ragsage/ingestion.py)) reports how many pages took each route —
...
([`classifier.py:66`](../src/ragsage/parsing/classifier.py)) disqualifies a page's text layer
...
transcription (`replace(page, text=text)`, [`ingestion.py:149-150`](../src/ragsage/ingestion.py)),
so good born-digital text is thrown away in favour of a model's reading of a 144-DPI render
(`_RENDER_SCALE`, [`pdf.py:56`](../src/ragsage/parsing/pdf.py)).
...
([`ingestion.py:68`](../src/ragsage/ingestion.py)). Raising `min_bitmap_area` (or
...
- `_measure_layout` ([`pdf.py:148`](../src/ragsage/parsing/pdf.py)) measures raster images
...
- Tuning the classifier cannot rescue it. Whether a page gets a rendered image is decided
  *inside the parser*, by a module-level classifier built with default thresholds
  (`_CLASSIFIER = LayoutPageClassifier()`, [`pdf.py:48`](../src/ragsage/parsing/pdf.py), used
  ...
  ([`classifier.py:62-64`](../src/ragsage/parsing/classifier.py)) — sensibly, since there
...
[`pdf.py:107-117`](../src/ragsage/parsing/pdf.py), with `_VISION_FALLBACK_LAYOUT`
...
`LLMClient.transcribe` ([`ports.py:126`](../src/ragsage/ports.py)) being able to resolve the
...
([`pdf.py:358`](../src/ragsage/parsing/pdf.py)) takes the most common rounded font size on the
page as the body size, and `_is_heading` ([`pdf.py:373`](../src/ragsage/parsing/pdf.py))
...
- **Chunk boundaries.** Pass 1 of the chunker splits on ATX headings
  (`_sections` [`chunking.py:118`](../src/ragsage/parsing/chunking.py), levels at
...
- **Contextualisation.** `HeadingWindowContextualizer` builds its context header from
  ...
  [`contextualizing.py:239`](../src/ragsage/contextualizing.py), read at
...
- **Supply DOCX or HTML where you can.** Those paths read declared structure, not typography:
  `Heading N` / `Title` styles (`_heading_level` [`docx.py:125`](../src/ragsage/parsing/docx.py))
  and `<h1>`…`<h6>` (`_HEADINGS` [`html.py:59`](../src/ragsage/parsing/html.py)). A document
...
([`ports.py:126`](../src/ragsage/ports.py)) actually calls a vision model. Routing works — a
...
`FakeLLMClient.transcribe` ([`fakes.py:307`](../src/ragsage/fakes.py)) decodes the image bytes
...
[`router.py:42`](../src/ragsage/parsing/router.py), `_EXTENSIONS` `router.py:58`,
...
([`backend.py:99-100`](../src/ragsage/parsing/backend.py)) — verified for `.doc`, `.xls` and
...
**Why it happens.** `route` ([`router.py:82`](../src/ragsage/parsing/router.py)) trusts the
...
([`models.py:53`](../src/ragsage/models.py)) and let the extension and the content sniff
...
- **A table with drawn rules coming through intact.** Rules are strong evidence, so those
  ...
  (`_ruled_tables`, [`pdf.py:415`](../src/ragsage/parsing/pdf.py)). Entry 2 is about the
...
- **Flow formats all landing on page 1.** DOCX, PPTX, HTML and text have no page grid;
  `_FLOW_FORMAT_PAGE` ([`backend.py:43`](../src/ragsage/parsing/backend.py)) collapses them
...
([`tests/test_pdf_path.py`](../tests/test_pdf_path.py)), so a reproduction can usually be
